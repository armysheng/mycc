...
- Claude provider 的主接入点放在 `MYCC_CLAUDE_*`：mycc 读取 `MYCC_CLAUDE_*` / 可选 `MYCC_CCR_*` / 兼容旧变量后，统一映射为 Claude/Anthropic runtime 需要的 `ANTHROPIC_BASE_URL`、`ANTHROPIC_AUTH_TOKEN`、`ANTHROPIC_API_KEY`。筑基已直接提供 Claude-compatible 模型时优先直连；CCR 只作为后续多 provider 路由、限流或审计层。不直接读取全局 `OPENAI_*`，避免和其他 OpenAI-compatible 用途混淆。
...
- 非 E2B Agent SDK runtime 的 cwd 限定在 `/home/{linuxUser}/workspace`，子进程 `HOME` 默认是 `/home/{linuxUser}`，`CLAUDE_CONFIG_DIR` 默认写到 `/home/{linuxUser}/.claude`。
...
- `npm run smoke:e2b-agent-workspace`：直接创建 E2B/code-server sandbox，并用 `e2b-claude-cli` runtime 在同一 `sandboxId` 内做 workspace 双向读写 smoke；同时验证 template 带 `code-server`、Claude CLI、Node/Python/Git/curl 和 GNU 常用工具链。
- `npm run smoke:e2b-agent-sdk-workspace`：同一 smoke 流程，但 agent runtime 切到 `e2b-claude-agent-sdk`，通过 sandbox 内 `/opt/mycc-agent-runtime/bridge.mjs` 调 `@anthropic-ai/claude-agent-sdk`；同时验证 Agent SDK bridge 文件和 GNU 常用工具链存在。
...
- `resolveClaudeProviderEnv()`：Agent SDK runtime 与 E2B Claude CLI runtime 共用 Claude provider env 解析。base URL 优先级为 `MYCC_CLAUDE_BASE_URL` > `MYCC_CCR_BASE_URL` > `MYCC_AGENT_SDK_BASE_URL` > `ANTHROPIC_BASE_URL` > `VPS_ANTHROPIC_BASE_URL`；credential 只转发第一组命中的 token/api key，避免 `ANTHROPIC_AUTH_TOKEN` 和旧 `ANTHROPIC_API_KEY` 同时下发。

当前实现刻意不把内部 `startCommand`、E2B host 或 traffic token 返回给客户端，避免前端依赖服务端执行细节或绕过后端代理。

## E2B Template 契约

当前主模板在 `mycc-sandbox/templates/e2b-assistant-sandbox`，旧 `mycc-backend/templates/e2b-code-server` 仅作为 code-server 过渡模板保留。主模板需要包含：
...
2. 用真实 E2B + `MYCC_CLAUDE_*` 直连凭据跑 `npm run smoke:e2b-agent-workspace`，验证 Claude CLI runtime 和 code-server 共享 `/home/mycc/workspace`，并覆盖 chat-first 自动创建 session 后 Remote IDE 复用的产品路径；该 smoke 也会检查 E2B template 的 GNU 工具链契约。
3. 用同一组凭据跑 `npm run smoke:e2b-agent-sdk-workspace`，验证 Agent SDK bridge runtime 也能复用同一 sandbox/workspace；该 smoke 会设置写文件所需的 SDK 工具/权限默认值，并检查 Agent SDK bridge 文件存在，产品默认仍保持只读工具集。
...
7. E2B agent runtime 下已停止从 VPS `/home/{linuxUser}/workspace` 注入项目上下文；chat context 和 E2B runtime 现在共用 `ensureE2bIdeSession`，会提前创建/复用同一个 E2B IDE session，并从 `/home/mycc/workspace/0-System/about-me` 读取 context。若全新 sandbox 还没有任何可用 about-me 内容，会跳过注入且不缓存 missing context；后续仍可补“首次创建 sandbox 时同步/生成 about-me 文件”。
8. E2B workspace/context 命令如果发现 running session 对应 sandbox 明确已失效（如 `sandbox not found` / `404` / stopped），会把 session 标记为 `stopped` 并提示重新打开 Remote IDE，避免反复命中坏 session；普通 timeout/执行错误不会误停 session。
