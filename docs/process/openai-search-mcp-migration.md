...
如果你的代理不需要 Bearer key：

```bash
OPENAI_ALLOW_EMPTY_API_KEY=true \
OPENAI_BASE_URL="http://YOUR_PROXY:PORT/v1" \
OPENAI_SEARCH_MODEL="gpt-4o-mini-search-preview" \
./scripts/migrate-key-test-users-openai-search.sh mycc_u2 mycc_u5 mycc_u9
```
...
1. 默认会把 `OPENAI_API_KEY` 写入用户 `~/.claude.json`。生产建议改为统一密钥注入方案（例如启动 wrapper 进程读取 root-only env 文件）。
2. 若使用无 key 代理，可设置 `OPENAI_ALLOW_EMPTY_API_KEY=true`。
3. 若代理仍走旧协议，可设置：
- `OPENAI_SEARCH_TOOL_TYPE=web_search_preview`
4. 该 MCP 已内置超时：
- `OPENAI_SEARCH_TIMEOUT_MS`（默认 45s）
5. 默认 command 使用绝对路径 `/usr/bin/node`，避免 PATH 差异导致连接失败。
