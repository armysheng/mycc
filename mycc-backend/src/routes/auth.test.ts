...
describe('auth routes', () => {
  ...
  it('redirects OAuth start requests to the provider authorization URL', async () => {
    mocks.buildOAuthAuthorizationUrl.mockReturnValueOnce('https://accounts.google.com/o/oauth2/v2/auth?client_id=google-client');
    const app = await buildApp();
    ...
    expect(response.headers.location).toBe('https://accounts.google.com/o/oauth2/v2/auth?client_id=google-client');
  ...
  it('redirects successful OAuth callbacks back to the frontend login bridge', async () => {
    ...
    expect(response.headers.location).toContain('oauth_token=oauth.jwt.token');
